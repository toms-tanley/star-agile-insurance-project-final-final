pipeline {
    ...
    stages {
        ...
        stage('Docker Image Build') {
            steps {
                sh 'docker build -t insurance123 .' // Build Docker image
            }
        }

        stage('Deploy to the Test Server') {
            steps {
                sh 'docker run -d -p 8084:8083 insurance123' // Run Docker container
        ...
        stage('Push Docker Image to Hub') {
            steps {
                ...
                sh 'docker tag insurance123 thomasdevops003/insure:v6'
